...
-> Under each JVM instance there will be heap memory
   ...
   | Concept                  | Meaning                                                           |
| ------------------------ | ----------------------------------------------------------------- |
| **Register**             | Fast CPU memory used to hold temporary values and enable fast ops |
| **Each thread has?**     | Its own set of registers (Program Counter, etc.)                  |
| **Context Switching**    | OS saves/restores registers to switch between threads             |
| **Program Counter (PC)** | Points to the current/next instruction for the thread             |
| **Managed by?**          | JVM and CPU manage all this automatically for each thread         |
